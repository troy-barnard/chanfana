...
// src/ui.ts
function getSwaggerUI(schemaUrl, docsPageTitle, faviconHref) {
...
function getReDocUI(schemaUrl, docsPageTitle, faviconHref, redocConfig) {
  schemaUrl = schemaUrl.replace(/\/+(\/|$)/g, "$1");
  return `<!DOCTYPE html>
    <html>
    <head>
...
var OpenAPIRoute = class {
  ...
  async validateRequest(request) {
    ...
    let validationSchema = z3.object(rawSchema);
...
var contentJson = (schema) => ({
  content: {
    "application/json": {
      schema: schema instanceof z4.ZodType ? schema : legacyTypeIntoZod(schema)
    }
  }
});
